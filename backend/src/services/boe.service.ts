...
class BOEService {
  private baseURL = 'https://www.boe.es/datos_abiertos/BOE';
  private lastRequestTime = 0;
  private minRequestInterval = 1000; // 1 segundo entre peticiones (rate limiting)
   ...
   */
  async search(params: BOEQueryParams): Promise<BOEResult[]> {
    ...
    try {
      ...
      if (params.q) {
        ...
        return results.filter(r => 
          ...
          (r.extracto && r.extracto.toLowerCase().includes(query))
      ...
      throw new ServiceException('BOE_ERROR', 'Error al consultar BOE', 503);
    }
  }
   ...
   */
  async getDocumento(id: string): Promise<BOEResult> {
    await this.rateLimit();
   ...
   */
  async searchByDateRange(fechaDesde: string, fechaHasta: string, query?: string): Promise<BOEResult[]> {
    ...
    if (diffDays > 30) {
      throw new ServiceException('DATE_RANGE_TOO_LARGE', 'El rango de fechas no puede superar 30 días', 400);
    }
   ...
   */
  async getUltimasDisposiciones(limit: number = 10): Promise<BOEResult[]> {
   ...
   */
  private parseSumarioXML(xml: string): BOEResult[] {
    const results: BOEResult[] = [];
   ...
   */
  private parseDocumentoXML(xml: string, id: string): BOEResult {
  ...
  private extractFechaFromId(id: string): string {
   ...
   */
  async syncWithDatabase(prisma: any, results: BOEResult[]): Promise<void> {
...
export const boeService = new BOEService();
