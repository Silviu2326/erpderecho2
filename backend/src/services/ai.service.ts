...
class AIService {
  ...
  async analizarCaso(input: AnalisisCasoInput): Promise<PrediccionResultado> {
    if (!this.isAvailable()) {
    ...
    try {
      ...
      if (!content) {
        throw new ServiceException('AI_ERROR', 'No se pudo obtener respuesta de la IA', 500);
      }
  ...
  async analizarSentimiento(texto: string): Promise<AnalisisSentimiento> {
    if (!this.isAvailable()) {
      return {
        sentimiento: 'neutral',
        confianza: 0.5,
  ...
  async generarEstrategia(input: AnalisisCasoInput): Promise<EstrategiaLegal> {
    if (!this.isAvailable()) {
  ...
  async resumirDocumento(texto: string, maxPalabras: number = 200): Promise<string> {
    if (!this.isAvailable()) {
  ...
  async extraerEntidadesLegales(texto: string): Promise<{
    fechas: string[];
    montos: string[];
    personas: string[];
  ...
  private buildAnalisisPrompt(input: AnalisisCasoInput): string {
...
Proporciona un análisis legal completo con predicción del resultado.`;
  ...
  private buildEstrategiaPrompt(input: AnalisisCasoInput): string {
...
¿Qué estrategia recomiendas?`;
  ...
  private parseAIResponse(content: string): PrediccionResultado {
    try {
  ...
  private getMockPrediccion(tipo: string): PrediccionResultado {
    const mockResultados: Record<string, PrediccionResultado> = {
      CIVIL: {
  ...
  private getMockEstrategia(): EstrategiaLegal {
    return {
