...
import {
  ConnectIntegracionDto,
  SendEmailDto,
  CalendarEventDto,
  SyncCalendarDto,
} from '../dtos/integraciones.dto';
...
async function getIntegracion(userId: string, tipo: 'MICROSOFT' | 'GOOGLE') {
...
async function isTokenExpired(integracion: any): Promise<boolean> {
  if (!integracion.expiresAt) return false;
  return new Date(integracion.expiresAt) <= new Date();
}

async function refreshTokenIfNeeded(integracion: any, tipo: 'MICROSOFT' | 'GOOGLE'): Promise<any> {
  if (await isTokenExpired(integracion)) {
    if (tipo === 'MICROSOFT') {
      const newTokens: any = await refreshMicrosoftToken(integracion.refreshToken);
      if (newTokens) {
        const expiresAt = new Date();
        expiresAt.setSeconds(expiresAt.getSeconds() + newTokens.expires_in);
        
        return prisma.integracion.update({
      ...
      const newTokens: any = await refreshGoogleToken(integracion.refreshToken);
      if (newTokens) {
        const expiresAt = new Date();
        expiresAt.setSeconds(expiresAt.getSeconds() + newTokens.expires_in);
...
router.get('/microsoft/status', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
...
router.post('/microsoft/connect', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const dto = plainToInstance(ConnectIntegracionDto, req.body) as ConnectIntegracionDto;
...
router.post('/microsoft/disconnect', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const integracion = await getIntegracion(req.user!.userId, 'MICROSOFT');
...
router.get('/microsoft/calendar', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const integracion = await getIntegracion(req.user!.userId, 'MICROSOFT');
    ...
    const response = await fetch(
      `https://graph.microsoft.com/v1.0/me/calendarView?startDateTime=${startDate}&endDateTime=${endDate}&$select=id,subject,body,start,end,location,attendees`,
      {
        headers: {
...
router.post('/microsoft/calendar/sync', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const dto = plainToInstance(SyncCalendarDto, req.body) as SyncCalendarDto;
    const errors = await validate(dto);
    if (errors.length > 0) {
      res.status(400).json({ success: false, error: { message: 'Validation failed', details: errors } });
      return;
...
router.post('/microsoft/email/send', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const dto = plainToInstance(SendEmailDto, req.body) as SendEmailDto;
    const errors = await validate(dto);
    if (errors.length > 0) {
      res.status(400).json({ success: false, error: { message: 'Validation failed', details: errors } });
      return;
...
router.get('/microsoft/onedrive', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const integracion = await getIntegracion(req.user!.userId, 'MICROSOFT');
...
router.post('/google/connect', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const dto = plainToInstance(ConnectIntegracionDto, req.body) as ConnectIntegracionDto;
    const errors = await validate(dto);
    if (errors.length > 0) {
...
router.get('/google/calendar', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const integracion = await getIntegracion(req.user!.userId, 'GOOGLE');
    ...
    const response = await fetch(
      `https://www.googleapis.com/calendar/v3/calendars/primary/events?timeMin=${new Date(startDate * 1000).toISOString()}&timeMax=${new Date(endDate * 1000).toISOString()}&singleEvents=true&orderBy=startTime`,
      {
        headers: {
          Authorization: `Bearer ${validIntegracion.accessToken}`,
        },
    ...
    res.json(formatResponse({ events: (data as any).items || [] }));
  } catch (error) {
    console.error('Google calendar error:', error);
    res.status(500).json(formatError('INTERNAL_ERROR', 'Error al obtener calendario'));
  }
...
router.post('/google/calendar/sync', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const dto = plainToInstance(SyncCalendarDto, req.body);
    const errors = await validate(dto);
    if (errors.length > 0) {
      res.status(400).json({ success: false, error: { message: 'Validation failed', details: errors } });
      return;
...
router.post('/google/email/send', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
...
router.get('/google/drive', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const integracion = await getIntegracion(req.user!.userId, 'GOOGLE');
    ...
    const response = await fetch(
      'https://www.googleapis.com/drive/v3/files?pageSize=20&orderBy=modifiedTime desc',
      {
        headers: {
          Authorization: `Bearer ${validIntegracion.accessToken}`,
        },
    ...
    const data = await response.json() as any;
    res.json(formatResponse({ files: (data as any).files || [] }));
  } catch (error) {
